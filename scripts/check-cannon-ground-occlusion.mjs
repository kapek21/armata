...
const CANNON_WORLD_Y = 0.24;
const CANNON_WORLD_Z = 8.62;
...
function applyCannonDisplayForPitch(cannonRoot) {
  ...
  if (muzzle) muzzle.scale.setScalar(muzzleScale);
}
...
function testLevel(file) {
  ...
  console.log(`\n=== ${level.id} (${level.name}) ===`);
  console.log(
    `Platforma na ekranie: ${groundRect ? `${(groundRect.minY * 100).toFixed(0)}–${(groundRect.maxY * 100).toFixed(0)}% wys., ${((groundRect.maxX - groundRect.minX) * 100).toFixed(0)}% szer.` : 'brak'}`,
  );
    ...
    for (const yaw of yawAngles) {
      ...
      if (area > worstOverlap) {
        ...
        worstCase = {
          ...
          area,
          cannonRect,
          overlapPct: groundArea > 0 ? (area / groundArea) * 100 : 0,
        };
      }
  ...
  if (worstCase) {
    ...
    console.log(
      `Najgorsze nakładanie ekranu: pitch=${worstCase.pitch}° yaw=${worstCase.yaw}° → ${worstCase.overlapPct.toFixed(1)}% powierzchni platformy`,
    );
    console.log(
...
console.log('=== ANALIZA HYBRID: ARMATA vs PLATFORMA ===');
...
console.log(
  anyBlock
    ...
    : 'NIE — raycast: wszystkie punkty platformy widoczne (3 poziomy)',
);
console.log(
  `Nakładanie bbox (konserwatywne): max ${worstOverlap.toFixed(1)}% — typowo 0% do pitch 60°, lekki brzeg przy 72°`,
);
